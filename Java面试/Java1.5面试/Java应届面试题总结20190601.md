...
作用：一个线程修改的状态对另一个线程是可见的。

本质：变量的内存可见性是基于内存屏障（Memory Barrier）实现。
...
@Configuration：xml=>javaConfig.class
...
集群master/slave选举机制，master处理所有请求，master选举机制保证分布式协调服务的高可用性。
